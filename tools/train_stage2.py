...
from bmds_net.models.bmds_net import BayesianBMDSNet
...
def main():
    ...
    det_cfg = dict(config['model'])
    det_cfg['name'] = 'bmds_net'
    det_cfg.pop('mc_dropout_rate', None)
    deterministic_model = create_model('bmds_net', **det_cfg)

    ckpt = torch.load(config['pretrained']['path'], map_location='cpu')
    state_dict = ckpt['model_state_dict'] if 'model_state_dict' in ckpt else ckpt
    clean_state = {k.replace('module.', ''): v for k, v in state_dict.items()}
    deterministic_model.load_state_dict(clean_state, strict=False)

    model = BayesianBMDSNet(
        deterministic_model,
        mc_dropout_rate=config['model'].get('mc_dropout_rate', 0.0),
    )
    model.to(device)
    ...
    train_loader = DataLoader(train_ds, batch_size=config['training']['batch_size'],
                            shuffle=True, num_workers=config['data'].get('num_workers', 4), pin_memory=True)
